...
function api({ orgKey, applicationKey, version } = {}) {
  return request.get(
    `${process.env.APIDOC_HOST}/${orgKey}/${applicationKey}/latest`
  );
...
const namespaceEntities = (namespace, entities, entityType) =>
  entities.map(entity =>
    Object.assign(entity, {
      name: `${namespace}.${entityType}.${entity.name}`,
      plural: `${namespace}.${entityType}.${entity.plural}`,
    })
  );
...
function* takeEverySaga() {
  yield* takeEvery(
    actionTypes.getByOrgkeyAndApplicationkeyAndVersion_get,
    saga
  );
}
